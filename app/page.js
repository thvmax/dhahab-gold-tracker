...
const GoldTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: "rgba(16,14,8,0.97)", border: "1px solid rgba(212,175,55,0.3)", borderRadius: 10, padding: "10px 16px" }}>
      <p style={{ color: "#8a7a50", fontSize: 11, marginBottom: 3 }}>{payload[0]?.payload?.date ? fmtDate(payload[0].payload.date) : ""}</p>
      <p style={{ color: "#D4AF37", fontSize: 16, fontWeight: 800 }}>AED {payload[0]?.value?.toFixed(2)}<span style={{ color: "#6a5a30", fontSize: 11, fontWeight: 400 }}>/g · 24K</span></p>
    </div>
  );
};

const LogModal = ({ initial, currentPrice, onSave, onClose }) => {
  const isEdit = !!initial?.id;
  const [form, setForm] = useState(
    initial
      ? { date: initial.date, grams: String(initial.grams), purchasePrice: String(initial.purchasePrice), fees: String(initial.fees || "") }
      : { date: "", grams: "", purchasePrice: "", fees: "" }
  );
  const [err, setErr] = useState("");

  const inp = {
    background: "rgba(255,255,255,0.04)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 10,
    color: "#e8dcc0", padding: "13px 14px", fontSize: 14, outline: "none", width: "100%",
  ...
  const handleSave = () => {
    ...
    onSave({ ...form, grams: parseFloat(form.grams), purchasePrice: parseFloat(form.purchasePrice), fees: parseFloat(form.fees) || 0 });
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center", background: "rgba(0,0,0,0.75)", backdropFilter: "blur(10px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#131009", border: "1px solid rgba(212,175,55,0.15)", borderRadius: "22px 22px 0 0", padding: "28px 22px 44px", width: "100%", maxWidth: 500, animation: "slideUp 0.28s cubic-bezier(0.34,1.4,0.64,1)" }}>
        ...
        {costPerGram && (
          <div style={{ background: "rgba(212,175,55,0.05)", border: "1px solid rgba(212,175,55,0.1)", borderRadius: 12, padding: "13px 16px", marginTop: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <p style={{ color: "#e8dcc0", fontWeight: 800, fontSize: 16, marginTop: 3 }}>AED {currentPrice}</p>
            </div>
            <div style={{ width: 1, height: 36, background: "rgba(212,175,55,0.1)" }} />
            <div style={{ textAlign: "right" }}>
              <p style={{ color: "#5a5040", fontSize: 10, letterSpacing: "1px" }}>DIFF/GRAM</p>
              {(() => {
                const diff = currentPrice - parseFloat(costPerGram);
                const pos = diff >= 0;
        ...
        <button onClick={handleSave} style={{ marginTop: 18, width: "100%", padding: 15, background: "linear-gradient(135deg,#D4AF37,#b8941f)", border: "none", borderRadius: 12, color: "#0d0b07", fontWeight: 800, fontSize: 15, cursor: "pointer" }}>
...
const LogCard = ({ log, currentPrice, onEdit, onDelete, confirmDelete, onCancelDelete }) => {
  ...
  const totalCost = log.purchasePrice + (log.fees || 0);
    ...
    <div style={{ background: confirmDelete ? "rgba(239,68,68,0.04)" : "rgba(255,255,255,0.022)", border: `1px solid ${confirmDelete ? "rgba(239,68,68,0.25)" : "rgba(212,175,55,0.1)"}`, borderRadius: 16, padding: "16px 18px", transition: "all 0.2s" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ color: "#4a3f28", fontSize: 11, marginTop: 3 }}>Bought @ AED {costPerGram}/g · Cost {fmtAED(totalCost)}</p>
        </div>
...
export default function App() {
  ...
  const [logs, setLogs] = useState([
    { id: 1, date: "2024-11-10", grams: 50, purchasePrice: 26000, fees: 350 },
    { id: 2, date: "2025-02-18", grams: 25, purchasePrice: 13800, fees: 150 },
  ]);
  const [modal, setModal] = useState(null);
  const [deleteConfirm, setDeleteConfirm] = useState(null);
  const [mounted, setMounted] = useState(false);
  const [currentPrice, setCurrentPrice] = useState(577.75);
  ...
  useEffect(() => {
    fetch("https://www.goldapi.io/api/XAU/AED", {
      ...
      .then(r => r.json())
      .then(d => {
        if (d && d.price_gram_24k) setCurrentPrice(d.price_gram_24k);
      })
  ...
  const totalGrams = logs.reduce((s, l) => s + l.grams, 0);
  const totalCost = logs.reduce((s, l) => s + l.purchasePrice + (l.fees || 0), 0);
  const totalValue = totalGrams * currentPrice;
  const totalPnL = totalValue - totalCost;
  const pnlPos = totalPnL >= 0;

  const handleSave = (data) => {
    if (modal.mode === "add") setLogs(p => [...p, { id: Date.now(), ...data }]);
    else setLogs(p => p.map(l => l.id === modal.log.id ? { ...l, ...data } : l));
    setModal(null);
  };

  const handleDeleteClick = (id) => {
    if (deleteConfirm === id) { setLogs(p => p.filter(l => l.id !== id)); setDeleteConfirm(null); }
    ...
    <div style={{ minHeight: "100vh", background: "#0d0b07", color: "#e8dcc0", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", paddingBottom: 100, opacity: mounted ? 1 : 0, transition: "opacity 0.5s" }}>
        ...
        <div style={{ maxWidth: 500, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 4, justifyContent: "flex-end" }}>
              <span style={{ color: "#D4AF37", fontWeight: 800, fontSize: 21 }}>AED {currentPrice.toFixed(2)}</span>
      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div className="au" style={{ marginTop: 24, padding: "24px 22px", background: "linear-gradient(145deg,rgba(212,175,55,0.08),rgba(212,175,55,0.02))", border: "1px solid rgba(212,175,55,0.18)", borderRadius: 22, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", top: -10, right: -10, width: 90, height: 90, borderRadius: "50%", border: "1px solid rgba(212,175,55,0.04)", pointerEvents: "none" }} />
          ...
          <p style={{ color: pnlPos ? "#4ade80" : "#f87171", fontSize: 14, fontWeight: 700, marginBottom: 20 }}>
            ...
            <span style={{ fontWeight: 400, fontSize: 12, color: pnlPos ? "#3a9060" : "#b05050" }}> unrealised {pnlPos ? "gain" : "loss"} · {pnlPos ? "+" : ""}{((totalPnL / totalCost) * 100).toFixed(1)}%</span>
          </p>
        ...
        <div className="au" style={{ marginTop: 14, background: "rgba(255,255,255,0.018)", border: "1px solid rgba(212,175,55,0.07)", borderRadius: 20, padding: "20px 4px 12px", animationDelay: "0.08s" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 16px", marginBottom: 14 }}>
            <div>
              ...
              <p style={{ color: "#4a3f28", fontSize: 11, marginTop: 1 }}>AED per gram · Dubai Gold & Jewellery Group</p>
            </div>
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div>
              ...
              <p style={{ color: "#4a3f28", fontSize: 11, marginTop: 2 }}>{logs.length} purchase{logs.length !== 1 ? "s" : ""} · 24K only · tap to edit or delete</p>
            </div>
          ...
          {logs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 20px", color: "#3a3020", border: "1px dashed rgba(212,175,55,0.09)", borderRadius: 18 }}>
              ...
              <p>No logs yet. Tap <strong style={{ color: "#D4AF37" }}>+ Add Log</strong> to record your first purchase.</p>
            </div>
          ) : (
        ...
        <div style={{ marginTop: 28, padding: "14px 18px", background: "rgba(212,175,55,0.025)", border: "1px solid rgba(212,175,55,0.06)", borderRadius: 14 }}>
          <p style={{ color: "#4a3f28", fontSize: 11, lineHeight: 1.8 }}>
            <span style={{ color: "#D4AF37" }}>⬡</span> All calculations use <strong style={{ color: "#6a5a30" }}>24K (999.9 purity)</strong> rate.<br />
            Live price: AED {currentPrice.toFixed(2)}/g · Source: GoldAPI.io<br />
            Chart prices are simulated history.
          </p>
        </div>
